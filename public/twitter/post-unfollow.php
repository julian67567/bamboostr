<?PHP
  ...
  if($status=="OK"){
    ...
    foreach($cola as $d){
      ...
      //$d[1] ids
      $ids[$condsf] = $d[1];
          $query = $conn->query("SELECT unfollow FROM limites_tw WHERE id_token='".$id_token."'");
          $row=$query->fetch_assoc();
          ...
          if(strtotime(date('d-m-Y H:i:s'))>=strtotime($limite_unfollow_array[1]) || $limite_unfollow==""){
            if($limite_unfollow==""){
              $conn->query("UPDATE `limites_tw` SET `unfollow`='1|".date('d-m-Y H:i:s', strtotime('-1 minutes'))."' WHERE `id_token`='".$id_token."'");
              $seguidores_cont++;
            } else {
              if($limite_unfollow_array2[0]==date("d-m-Y")){
                if($limite_unfollow_array[0]<500 && ($tipo=="pro" || $tipo=="ent" || $tipo=="basic")){
                  $conn->query("UPDATE `limites_tw` SET `unfollow`='".($limite_unfollow_array[0]+1)."|".date('d-m-Y H:i:s', strtotime('-1 minutes'))."' WHERE `id_token`='".$id_token."'");
                  $seguidores_cont++;
