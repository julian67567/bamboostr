...
class TwitterOAuth {
  ...
  function http($url, $method, $postfields = NULL) {
	...
	/*No poner porque cierra conexiones tardadas como por ejemplo el filtro*/
	...
	/* -s quita metricas y otras basuras 
	
	exec('php /path/to/curlscript.php > /dev/null &');
	$exec_query=exec($cmd, $response, $exit);
	...
	/*
	$cmd = "curl -s -X ".$method." -H 'Content-Type: application/json' '".$url."' /dev/null 2>&1 & ";  
	$exec_query=exec($cmd, $response, $exit);
	$response=$response[0];
	*/
