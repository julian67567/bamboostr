...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
    if(!$identify){
      //por día
      if(strpos($row["muestreo"],date('d-m-Y'))===false){
      ...
      //por hora
      if(strpos($row["muestreo"],date('d-m-Y:H'))===false){
        if(strpos($unique_request,$row["screen_name"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["oauth_token"];
	      $cuentas[$c][2] = $row["oauth_token_secret"];
  ...
  foreach($cuentas as $item){
	...
	if($twitteroauth->http_code==200){
	    $seguidores_rango = $user_info->followers_count;
		$c=1;
		...
		while($group<=count($grupo_100)){
		  ...
		  $group++;
                ...
                $archivo = fopen("../usuarios/".$item[0]."/muestreo.txt", "rb");
                if($archivo){
                ...
                if(count($muestreo_array_arch)<=0){
                  ...
                  $archivo = fopen("../usuarios/".$item[0]."/muestreo.txt", "w+");
                  ...
                  //compara los anteriores ids en la "BD" haber si siguen siendo seguidores y actualiza los usuario con info anterior por la nueva info. 
                  $keyA = array();
                  foreach($seguidores_array as $sdfgew){
                    $keyA["".$sdfgew.""] = 1;
                  }
                  $archivo3 = fopen("../usuarios/".$item[0]."/muestreo.txt", "w+");
                  ...
                  $archivo2 = fopen("../usuarios/".$item[0]."/muestreo.txt", "a+");
