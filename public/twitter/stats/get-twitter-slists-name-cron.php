...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	if(!$identify){
      if(strpos($row["slistasname"],date('d-m-Y'))===false){
	  ...
	  if(strpos($row["slistasname"],date('d-m-Y:H'))===false){
	    if(strpos($unique_request,$row["screen_name"])===false){
	      $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["oauth_token"];
	      $cuentas[$c][2] = $row["oauth_token_secret"];
  ...
  foreach($cuentas as $item){
	...
	if($twitteroauth->http_code==200){
		...
		$cont = 1;
		  ...
		  $mlists = $twitteroauth->get("https://api.twitter.com/1.1/lists/list.json?screen_name=".$item[0]."");
		...
		$query2=$conn->query("UPDATE estadisticas_twitter SET slistasname='".$listas."' 
