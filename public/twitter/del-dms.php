<?PHP
  ...
  if($status=="OK"){
	  ...
	  //DMS Mandados
	  $GETDM_MAN = $twitteroauth->get("https://api.twitter.com/1.1/direct_messages/sent.json?count=200");
	  foreach($GETDM_MAN as &$item){
            echo ''.$item->{'id'}.' '.$item->{'screen_name'}.' ';
	    //Destruye DMS
	    $twitteroauth->post("https://api.twitter.com/1.1/direct_messages/destroy.json?id=".$item->{'id'}."");
	    echo "<br />";
	  }
