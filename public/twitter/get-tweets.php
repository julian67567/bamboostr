<?PHP
  ...
  if($_GET["oauth_verifier"] && $_SESSION['oauth_token'] && $_SESSION['oauth_token_secret']){
	...
	if($query->num_rows>0){
	  ...
	  $query = $conn->query("SELECT oauth_token,oauth_token_secret FROM token WHERE screen_name='".$argv[1]."'") or die(mysqli_error($conn));
	  if($query->num_rows>0){
  ...
  if($status=="OK"){
	...
	$max_id = $argv[3];
	  if(!$max_id){
	    //MAXID devuelve mensajes viejos a partir de ese mensaje
        $tweets = $twitteroauth->get("https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=".$screen_name."&count=".$count."");
	  } else {
		$tweets = $twitteroauth->get("https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=".$screen_name."&count=".$count."&max_id=".$max_id."");
	  }
