  ...
  if($status=="OK"){
    ...
    foreach($cola as $d){
      ...
      if( (intval(($seguidores)*(1.1))>$siguiendo) || ($seguidores<=5000 && $siguiendo<5000)){
          $query = $conn->query("SELECT follow FROM limites_tw WHERE id_token='".$id_token."'") or die(mysqli_error($conn));
          $row=$query->fetch_assoc();
          ...
          if(strtotime(date('d-m-Y H:i:s'))>=strtotime($limite_follow_array[1]) || $limite_follow==""){
            if($limite_follow==""){
              $conn->query("UPDATE `limites_tw` SET `follow`='1|".date('d-m-Y H:i:s', strtotime('-1 minutes'))."' WHERE `id_token`='".$id_token."'") or die(mysqli_error($conn));
              $seguidores_cont++;
            } else {
              if($limite_follow_array2[0]==date("d-m-Y")){
                if($limite_follow_array[0]<975 && ($tipo=="pro" || $tipo=="ent" || $tipo=="basic")){
                  $conn->query("UPDATE `limites_tw` SET `follow`='".($limite_follow_array[0]+1)."|".date('d-m-Y H:i:s', strtotime('-1 minutes'))."' WHERE `id_token`='".$id_token."'") or die(mysqli_error($conn));
                  $seguidores_cont++;
