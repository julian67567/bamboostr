<?PHP
  ...
  if($_GET["oauth_verifier"] && $_SESSION['oauth_token'] && $_SESSION['oauth_token_secret']){
	...
	if($query->num_rows>0){
	  ...
	  $query = $conn->query("SELECT identify FROM token WHERE id='".$argv[3]."'");
	  $row=$query->fetch_assoc();
          $identify = $row["identify"]; 

	  //si no hay credenciales en la url
	  $query = $conn->query("SELECT oauth_token,oauth_token_secret,id,tipo,identify FROM token WHERE screen_name='".$argv[1]."'");
	  if($query->num_rows>0){
	    $row=$query->fetch_assoc();
    ...
    $status = 'ERROR';
  if($status=="OK"){
          $query2 = $conn->query("SELECT * FROM limites_tw WHERE id_token='".$id_token."'");
          $conn->close();
