...
if($userPage==""){
  ...
  $query=$conn->query("SELECT access_token FROM social_share WHERE identify='".$userPage."' AND red='facebook' AND tipo='page' order by id DESC") or die(mysqli_error($conn));
}
if($query->num_rows>0){
	...
	try {
	  ...
	  if($until && $userPage){
	    $request = new FacebookRequest($session, 'GET', '/'.$userPage.'/conversations?until='.$until.'&limit='.$count.'');
	  } else if($userPage) {
	    $request = new FacebookRequest($session, 'GET', '/'.$userPage.'/conversations?limit='.$count.'');
          }

	  if($until && $userPage==""){
	    $request = new FacebookRequest($session, 'GET', '/me/inbox?until='.$until.'&limit='.$count.'');
	  } else if($userPage=="") {
	    $request = new FacebookRequest($session, 'GET', '/me/inbox?limit='.$count.'');
          }
