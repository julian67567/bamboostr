/*probar si funciona facebook insights*/
<?php
//PHP Version 5.4.34
include '../../conexioni.php';
$identify=10152844365711702;
$identify_account=1671291236430973;
session_start();
...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	/**DIFerencia de MINUTOS ENTRE PETICIONES**/
      $difFechaPass = '';
	  //17 inlcluye ,
	  $difFechaPass=substr($row["fans_country"],strlen($row["fans_country"])-17,16);
	...
	if(!$identify){
	  //por día si NO fue directo del modulo stats
      if(strpos($row["fans_country"],date("d-m-Y",strtotime("-2 day")))===false && ($difFechaPass>15 && $difFechaPass<1425) && $row["last_ssid"]!=""){
	  ...
	  //por hora directo del modulo stats
	  if(strpos($row["fans_country"],date("d-m-Y:H",strtotime("-2 day")))===false || ($difFechaPass>15 && $difFechaPass<1425)){
        if(strpos($unique_request,$row["identifyP"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["access_token"];
	      $cuentas[$c][2] = $row["id_token"];
  ...
  foreach($cuentas as $item){
	...
	try {
	  ...
	  /*$request = new FacebookRequest($session, 'GET', '/'.$item[4].'/insights/page_fans_country/lifetime?until='.date("Y-m-d",strtotime("-1 day")).'&since='.date("Y-m-d",strtotime("-2 day")).'');
          */
          $request = new FacebookRequest($session, 'GET', '/'.$item[4].'/insights/page_fans_country/lifetime');
	...
	if($error==0) {
	  ...
	  foreach($req[0]->values as $item2){
		...
		$keys = array();
		$keys = array_keys((array)$item2->value);
		$c=0;
		foreach($item2->value as $item3){
	      $fans_country = ''.$fans_country.''.$item3.'|'.str_replace(',', '',$keys[$c]).',';
		  $c++;
		}
		$fans_country = ''.$fans_country.''.$fecha.',';
	  }
	  
	   //si no hay el mismo día
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET fans_country='".$fans_country."'
							   WHERE identify='".$item[4]."' AND red='facebook'
							   AND tipo='page'");
		 
		  echo '<br />Total: '.$fans_country.' '.$item[4].', '.$item[0].'<br />';
