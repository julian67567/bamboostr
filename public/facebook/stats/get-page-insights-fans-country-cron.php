...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	...
	if(!$identify){
	  //por día
      if(strpos($row["fans_country"],date("d-m-Y",strtotime("-2 day")))===false && $difFechaPass>15){
	  ...
	  if(strpos($row["fans_country"],date("d-m-Y:H",strtotime("-2 day")))===false || $difFechaPass>15){
        if(strpos($unique_request,$row["identifyP"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["access_token"];
	      $cuentas[$c][2] = $row["id_token"];
