...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	/**DIFerencia de MINUTOS ENTRE PETICIONES**/
      $difFechaPass = '';
	  //17 inlcluye ,
	  $difFechaPass=substr($row["admin_num_posts"],strlen($row["admin_num_posts"])-17,16);
	...
	if(!$identify){
	  //por día
      if(strpos($row["admin_num_posts"],date("d-m-Y",strtotime("-1 day")))===false && $difFechaPass>15){
        if(strpos($unique_request,$row["identifyP"])===false){
		  ...
		  $cuentas[$c][3] = $row["admin_num_posts"];
	  ...
	  if(strpos($row["admin_num_posts"],date("d-m-Y:H",strtotime("-1 day")))===false || $difFechaPass>15){
        if(strpos($unique_request,$row["identifyP"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["access_token"];
	      $cuentas[$c][2] = $row["id_token"];
  ...
  foreach($cuentas as $item){
	...
	try {
	  ...
	  //?period=month
	  $request = new FacebookRequest($session, 'GET', '/'.$item[4].'/insights/page_admin_num_posts/day?until='.date("Y-m-d").'&since='.date("Y-m-d",strtotime("-1 day")).'');
	...
	if($error==0) {
	  ...
	  foreach($req[0]->values as $item2){
	    ...
	    $admin_num_posts = ''.$admin_num_posts.''.$item2->value.'|'.$fecha.',';
	  }
	  if(strpos($cuentas[$c][3],date("d-m-Y",strtotime("-1 day")))===false){
		  ...
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET admin_num_posts=CONCAT(admin_num_posts,'".$admin_num_posts."')
							   WHERE identify='".$item[4]."' AND red='facebook'
							   AND tipo='page'") or die(mysqli_error($conn));
		  
		  echo 'No hay el Mismo Día. Total: '.$admin_num_posts.' '.$item[4].', '.$item[0].'<br />';
		  
	  } else {
		  //si hay el mismo día
		  $admin_num_posts_array=explode(",",$cuentas[$c][3]);
		  for($i=0; $i<count($admin_num_posts_array)-2; $i++){
		    $admin_num_posts2=''.$admin_num_posts2.''.$admin_num_posts_array[$i].',';
		  }
		  $cuentas[$c][3] = ''.$admin_num_posts2.''.$admin_num_posts.'';
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET admin_num_posts='".$cuentas[$c][3]."'
							   WHERE identify='".$item[4]."' AND red='facebook'
							   AND tipo='page'") or die(mysqli_error($conn));
		  
		  echo 'Si hay el mismo día. Total: '.$admin_num_posts.' '.$item[4].', '.$item[0].'<br />';
