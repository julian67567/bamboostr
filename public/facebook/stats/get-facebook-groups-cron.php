...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	/**DIFerencia de MINUTOS ENTRE PETICIONES**/
      $difFechaPass = '';
	  //17 inlcluye ,
	  $difFechaPass=substr($row["groups"],strlen($row["groups"])-17,16);
	...
	if(!$identify){
	  //por día
      if(strpos($row["groups"],date('d-m-Y'))===false && $difFechaPass>15){
	  ...
	  if(strpos($row["groups"],date('d-m-Y:H'))===false || $difFechaPass>15){
        if(strpos($unique_request,$row["screen_name"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["access_token"];
	      $cuentas[$c][2] = $row["id_token"];
  ...
  foreach($cuentas as $item){
	...
	if($error==0) {
      ...
      if(strpos($cuentas[$c][3],date('d-m-Y'))===false){
		  ...
		  //si hay el mismo día
		  $groups_array=explode(",",$cuentas[$c][3]);
		  for($i=0; $i<count($groups_array)-2; $i++){
		    $groups=''.$groups.''.$groups_array[$i].',';
		  }
		  $cuentas[$c][3] = ''.$groups.''.$contGroup.'|'.date('d-m-Y:H-i').',';
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET groups='".$cuentas[$c][3]."'
							   WHERE identify_account='".$item[4]."' AND red='facebook'
							   AND tipo='facebook'") or die(mysqli_error($conn));
