...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	/**DIFerencia de MINUTOS ENTRE PETICIONES**/
      $difFechaPass = '';
	  //17 inlcluye ,
	  $difFechaPass=substr($row["likes_to_pages"],strlen($row["likes_to_pages"])-17,16);
	...
	if(!$identify){
	  //por día
      if(strpos($row["likes_to_pages"],date('d-m-Y'))===false && $difFechaPass>15){
        if(strpos($unique_request,$row["screen_name"])===false){
		  ...
		  $cuentas[$c][3] = $row["likes_to_pages"];
	  ...
	  if(strpos($row["likes_to_pages"],date('d-m-Y:H'))===false || $difFechaPass>15){
        if(strpos($unique_request,$row["screen_name"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["access_token"];
	      $cuentas[$c][2] = $row["id_token"];
	...
	if($error==0) {
	  ...
	  if($graphObject->getProperty('data')){
	    foreach($graphObject->getProperty('data')->asArray() as $item2){
	      $likes_names = ''.$likes_names.''.str_replace(',', '', str_replace('|', '', $item2->name)).'|'.$item2->id.',';
		  $contLike++;
	    }
	  }
	  $likes_names = ''.$likes_names.''.date('d-m-Y:H-i').'';
	  $query2=$conn->query("UPDATE estadisticas_facebook 
	                       SET likes_name='".$likes_names."' 
						   WHERE identify_account='".$item[4]."' AND red='facebook'
						   AND tipo='facebook'") or die(mysqli_error($conn));
	  
	  echo '<br /><br />'.$cuentas[$c][0].' '.$contLike.' '.$likes_names.'<br /><br />';
	  
      if(strpos($cuentas[$c][3],date('d-m-Y'))===false){
		  //si no hay el mismo día
		  $cuentas[$c][3] = ''.$contLike.'|'.date('d-m-Y:H-i').',';
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET likes_to_pages=CONCAT(likes_to_pages,'".$cuentas[$c][3]."')
							   WHERE identify_account='".$item[4]."' AND red='facebook'
							   AND tipo='facebook'") or die(mysqli_error($conn));
		  ...
		  //si hay el mismo día
		  $likes_array=explode(",",$cuentas[$c][3]);
		  for($i=0; $i<count($likes_array)-2; $i++){
		    $likes=''.$likes.''.$likes_array[$i].',';
		  }
		  $cuentas[$c][3] = ''.$likes.''.$contLike.'|'.date('d-m-Y:H-i').',';
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET likes_to_pages='".$cuentas[$c][3]."'
							   WHERE identify_account='".$item[4]."' AND red='facebook'
							   AND tipo='facebook'") or die(mysqli_error($conn));
