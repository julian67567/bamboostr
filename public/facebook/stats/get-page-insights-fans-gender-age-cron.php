...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	/**DIFerencia de MINUTOS ENTRE PETICIONES**/
      $difFechaPass = '';
	  //17 inlcluye ,
	  $difFechaPass=substr($row["fans_gender_age"],strlen($row["fans_gender_age"])-17,16);
	...
	if(!$identify){
	  //por día
      if(strpos($row["fans_gender_age"],date("d-m-Y",strtotime("-2 day")))===false && $difFechaPass>15){
        if(strpos($unique_request,$row["identifyP"])===false){
		  ...
		  $cuentas[$c][3] = $row["fans_gender_age"];
	  ...
	  if(strpos($row["fans_gender_age"],date("d-m-Y:H",strtotime("-2 day")))===false || $difFechaPass>15){
        if(strpos($unique_request,$row["identifyP"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["access_token"];
	      $cuentas[$c][2] = $row["id_token"];
  ...
  foreach($cuentas as $item){
	...
	try {
	  ...
	  $request = new FacebookRequest($session, 'GET', '/'.$item[4].'/insights/page_fans_gender_age/lifetime?until='.date("Y-m-d",strtotime("-1 day")).'&since='.date("Y-m-d",strtotime("-2 day")).'');
	...
	if($error==0) {
	  ...
	  foreach($req[0]->values as $item2){
		...
		foreach($item2->value as $item3){
	      $fans_gender_age = ''.$fans_gender_age.''.$item3.'|'.str_replace(',', '',$keys[$c]).',';
		  $c++;
		}
		$fans_gender_age = ''.$fans_gender_age.''.$fecha.',';
	  }
	  
	   //si no hay el mismo día
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET fans_gender_age='".$fans_gender_age."'
							   WHERE identify='".$item[4]."' AND red='facebook'
							   AND tipo='page'");
		 
		  echo '<br />Total: '.$fans_gender_age.' '.$item[4].', '.$item[0].'<br />';
