...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	/**DIFerencia de MINUTOS ENTRE PETICIONES**/
      $difFechaPass = '';
	  //17 inlcluye ,
	  $difFechaPass=substr($row["likes_remove"],strlen($row["likes_remove"])-17,16);
	...
	if(!$identify){
	  //por día
      if(strpos($row["likes_remove"],date("d-m-Y",strtotime("-1 day")))===false && $difFechaPass>15){
	  ...
	  if(strpos($row["likes_remove"],date("d-m-Y:H",strtotime("-1 day")))===false || $difFechaPass>15){
        if(strpos($unique_request,$row["identifyP"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["access_token"];
	      $cuentas[$c][2] = $row["id_token"];
  ...
  foreach($cuentas as $item){
	...
	try {
	  ...
	  //?period=month
	  $request = new FacebookRequest($session, 'GET', '/'.$item[4].'/insights/page_fan_removes_unique/day?until='.date("Y-m-d").'&since='.date("Y-m-d",strtotime("-1 day")).'');
	...
	if($error==0) {
	  ...
	  foreach($req[0]->values as $item2){
	    ...
	    $likes_remove = ''.$likes_remove.''.$item2->value.'|'.$fecha.',';
	  }
	  if(strpos($cuentas[$c][3],date("d-m-Y",strtotime("-1 day")))===false){
		  ...
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET likes_remove=CONCAT(likes_remove,'".$likes_remove."')
							   WHERE identify='".$item[4]."' AND red='facebook'
							   AND tipo='page'");
		  
		  echo 'No hay el Mismo Día. Total: '.$likes_remove.' '.$item[4].', '.$item[0].'<br />';
		  
	  } else {
		  //si hay el mismo día
		  $likes_array_remove=explode(",",$cuentas[$c][3]);
		  for($i=0; $i<count($likes_array_remove)-2; $i++){
		    $remove2=''.$remove2.''.$likes_array_remove[$i].',';
		  }
		  $cuentas[$c][3] = ''.$remove2.''.$likes_remove.'';
		  $query2=$conn->query("UPDATE estadisticas_facebook 
							   SET likes_remove='".$cuentas[$c][3]."'
							   WHERE identify='".$item[4]."' AND red='facebook'
							   AND tipo='page'");
		  
		  echo 'Si hay el mismo día. Total: '.$likes_remove.' '.$item[4].', '.$item[0].'<br />';
