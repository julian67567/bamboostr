...
if($query->num_rows>0){
  ...
  while($row=$query->fetch_assoc()){
	/**DIFerencia de MINUTOS ENTRE PETICIONES**/
      $difFechaPass = '';
	  //17 inlcluye ,
	  $difFechaPass=substr($row["fans_locale"],strlen($row["fans_locale"])-17,16);
	...
	if(!$identify){
	  //por día
      if(strpos($row["fans_locale"],date("d-m-Y",strtotime("-2 day")))===false && ($difFechaPass>15 && $difFechaPass<1425)){
	  ...
	  if(strpos($row["fans_locale"],date("d-m-Y:H",strtotime("-2 day")))===false || ($difFechaPass>15 && $difFechaPass<1425)){
        if(strpos($unique_request,$row["identifyP"])===false){
		  $cuentas[$c][0] = $row["screen_name"];
	      $cuentas[$c][1] = $row["access_token"];
	      $cuentas[$c][2] = $row["id_token"];
  ...
  foreach($cuentas as $item){
	...
	try {
	  ...
	  //?period=month
	  $request = new FacebookRequest($session, 'GET', '/'.$item[4].'/insights/page_fans_locale/lifetime?until='.date("Y-m-d",strtotime("-1 day")).'&since='.date("Y-m-d",strtotime("-2 day")).'');
