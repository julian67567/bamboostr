...
if($secundaria=="no"){
	...
	if($oauth_token && $oauth_token_secret){
		...
		$query = $conn->query("SELECT id,ssid FROM token WHERE identify='".$user_info->id_str."' AND red='twitter'") OR DIE(mysqli_error($conn));
		if($query->num_rows>0){
		  //si existe usuario
		  //get id from token
		  ...
		  $query2=$conn->query("UPDATE token SET expire_token=0, identify='".$user_info->id_str."', ssid='APP".session_id()."', foto='".$user_info->profile_image_url."', oauth_token='".$oauth_token."', oauth_token_secret='".$oauth_token_secret."', last_ssid='".date("d-m-Y")."' WHERE identify='".$user_info->id_str."' AND red='twitter'") OR DIE(mysqli_error($conn));
		} else { 
		  //si no existe usuario
		  $query2=$conn->query("INSERT INTO token (identify,red,ssid,foto,screen_name_bamboostr,oauth_token,oauth_token_secret,social_networks,idioma,first_ssid,last_ssid) VALUES ('".$user_info->id_str."','twitter','APP".session_id()."','".$user_info->profile_image_url."','".$user_info->screen_name."','".$oauth_token."','".$oauth_token_secret."','tw".$user_info->id_str.",','".getUserLanguage()."','".date("d-m-Y")."','".date("d-m-Y")."')") OR DIE(mysqli_error($conn));
		  
		  $query2 = $conn->query("SELECT id FROM token WHERE identify='".$user_info->id_str."' AND red='twitter'") OR DIE(mysqli_error($conn));
		  ...
		  $id_token = $row["id"];
                  ...
                  $conn->query("INSERT INTO limites_tw (id_token) VALUES ('".$id_token."')") OR DIE(mysqli_error($conn));
		  ...
		  $conn->query("INSERT INTO notificaciones (id_token,receptor,titulo,mensaje,fecha,red) 
		              VALUES ('".$id_token."','".$identify."','Bienvenid@ a Bamboostr','".utf8_decode($bodyNot)."','".date("d-m-Y H:i")."','twitter')") OR DIE(mysqli_error($conn));
  ...
  if($identify){
	...
	if($user_info){
	  ...
	  //Almacenar tokens en la base de datos
		...
		if($query->num_rows>0){
		  $query2=$conn->query("UPDATE token SET expire_token=0, foto='".$user_info->profile_image_url."', oauth_token='".$oauth_token."', oauth_token_secret='".$oauth_token_secret."' WHERE identify='".$user_info->id_str."' AND red='twitter'") OR DIE(mysqli_error($conn));
		} else {
		  $query2=$conn->query("INSERT INTO token (identify, social_networks,red, idioma, foto,screen_name_bamboostr,oauth_token,oauth_token_secret) VALUES ('".$user_info->id_str."','tw".$user_info->id_str.",','twitter','".getUserLanguage()."','".$user_info->profile_image_url."','".$user_info->screen_name."','".$oauth_token."','".$oauth_token_secret."')") OR DIE(mysqli_error($conn));
                  ...
                  $bodyNot = 'Gracias por tu interés, estamos seguros que con bamboostr podrás utilizar tus Redes Sociales de la mejor manera para poder conseguir más clientes potenciales y hacer crecer tu negocio. <br /><br />Aquí encontrarás las herramientas necesarias para  posicionar tu marca en este importante canal, que día con día cobra mayor impacto. <br /><br />Para cualquier aclaración, duda o sugerencia escribanos en nuestras redes sociales o envíe un mensaje a soporte@bamboostr.com';
		  $conn->query("INSERT INTO notificaciones (id_token,receptor,titulo,mensaje,fecha,red) 
		              VALUES ('".$row['id']."','".$user_info->id_str."','Bienvenid@ a Bamboostr','".utf8_decode($bodyNot)."','".date("d-m-Y H:i")."','twitter')") OR DIE(mysqli_error($conn));
		}
		//agregar usuario la variable $red en social_networks para agregar cuentas secundarias en las primarias
		$query = $conn->query("SELECT social_networks,id,tipo FROM token WHERE identify='".$identify."' AND red='".$red."'") OR DIE(mysqli_error($conn));
		$row=$query->fetch_assoc();
		...
		if(strpos($social_networks, $user_info->id_str)===false){
		  ...
		  $query2=$conn->query("UPDATE token SET social_networks='".$social_networks."tw".$user_info->id_str.",' WHERE identify='".$identify."' AND red='".$red."'") OR DIE(mysqli_error($conn));
		  $query2=$conn->query("INSERT INTO grupos (user,grupo,id_token,tipo) VALUES ('".$user_info->id_str."','".$identify."','".$id_token."','".$tipo."')") OR DIE(mysqli_error($conn));
		  //estadisticas
